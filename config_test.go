...
func TestRun_ExplicitPort(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cfg := ParseArgs([]string{"--port", strconv.Itoa(tt.port)}, nil)
			if cfg.Port != tt.port {
...
func TestRun_Port443AutoTLS(t *testing.T) {
	cfg := ParseArgs([]string{"--port", "443"}, nil)
	if cfg.Port != 443 {
...
func TestRun_Port8443AutoTLS(t *testing.T) {
	cfg := ParseArgs([]string{"--port", "8443"}, nil)
	if cfg.Port != 8443 {
...
func TestRun_TLSWithCustomPort(t *testing.T) {
	cfg := ParseArgs([]string{"--tls", "--port", "9000"}, nil)
	if cfg.Port != 9000 {
...
func TestRun_TLSPortOrderIndependent(t *testing.T) {
	cfg1 := ParseArgs([]string{"--tls", "--port", "9000"}, nil)
	cfg2 := ParseArgs([]string{"--port", "9000", "--tls"}, nil)
...
func TestRun_CustomCerts(t *testing.T) {
	cfg := ParseArgs([]string{"--cert", "/path/cert.pem", "--key", "/path/key.pem"}, nil)
	if !cfg.TLSEnabled {
...
func TestRun_TLSDisable(t *testing.T) {
	cfg := ParseArgs([]string{"--port", "443", "--tls-disable"}, nil)
	if cfg.TLSEnabled {
...
func TestRun_OtherPortsNoTLS(t *testing.T) {
	for _, port := range []int{80, 3000, 5000, 8080, 10000} {
		cfg := ParseArgs([]string{"--port", strconv.Itoa(port)}, nil)
		if cfg.TLSEnabled {
...
func TestRun_RecentMax(t *testing.T) {
	cfg := ParseArgs([]string{"--recent", "25"}, nil)
	if cfg.RecentMax != 25 {
...
func TestRun_ListRecentPublic(t *testing.T) {
	cfg := ParseArgs([]string{"--list-recent-public"}, nil)
...
func TestInstall_DefaultPort(t *testing.T) {
	cfg := ParseArgs([]string{"install"}, nil)
...
func TestInstall_ExplicitPort443(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--port", "443"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestInstall_ExplicitPort8443(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--port", "8443"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestInstall_ExplicitPort8080(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--port", "8080"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestInstall_ExplicitPort3000(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--port", "3000"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestInstall_ExplicitTLS(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--tls"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestInstall_TLSDisable(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--port", "443", "--tls-disable"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestInstall_CustomCerts(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--cert", "/certs/cert.pem", "--key", "/certs/key.pem"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestInstall_Port8080WithTLSCerts(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--port", "8080", "--cert", "/certs/cert.pem", "--key", "/certs/key.pem"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestInstall_Combined(t *testing.T) {
	cfg := ParseArgs([]string{"install", "--port", "3000", "--recent", "5"}, nil)
	installCfg := cfg.InstallConfig()
...
func TestUninstall_Command(t *testing.T) {
	cfg := ParseArgs([]string{"uninstall"}, nil)
...
func TestUninstall_WithRemoveBinary(t *testing.T) {
	cfg := ParseArgs([]string{"uninstall", "--remove-binary"}, nil)
	if cfg.Command != "uninstall" {
...
func TestRun_PortBelow1024(t *testing.T) {
	cfg := ParseArgs([]string{"--port", "80"}, nil)
	if cfg.Port != 80 {
...
func TestRun_DuplicatePorts(t *testing.T) {
	cfg := ParseArgs([]string{"--port", "3000", "--port", "9000"}, nil)
	if cfg.Port != 9000 {
...
func TestRun_UnknownFlagsIgnored(t *testing.T) {
	cfg := ParseArgs([]string{"--unknown", "value", "--port", "9000"}, nil)
	if cfg.Port != 9000 {
...
func TestRun_AvailablePorts(t *testing.T) {
	// With explicit port, available ports should be ignored
	cfg := ParseArgs([]string{"--port", "3000"}, []int{8080, 8081})
	if cfg.Port != 3000 {
		t.Errorf("Explicit port should be used, got %d", cfg.Port)
	}
}

func TestRun_UseFirstAvailablePort(t *testing.T) {
	// Without explicit port, first available should be used
	cfg := ParseArgs([]string{}, []int{8081, 8082})
	if cfg.Port != 8081 {
		t.Errorf("First available port should be used, got %d", cfg.Port)
	}
}

func TestRun_EmptyAvailablePorts(t *testing.T) {
	// With no available ports and no explicit, should use default
	cfg := ParseArgs([]string{}, []int{})
	if cfg.Port != 8080 {
		t.Errorf("Default port should be used when no available, got %d", cfg.Port)
	}
}

func TestRun_NilAvailablePorts(t *testing.T) {
	// With nil available ports, should use default
	cfg := ParseArgs([]string{}, nil)
	if cfg.Port != 8080 {
		t.Errorf("Default port should be used when nil, got %d", cfg.Port)
	}
}
